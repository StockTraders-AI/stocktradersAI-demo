...
const CHAT_HISTORY_STORAGE_KEY = "st:market-flow:tu-van-ai-chat:v1";
...
const TEXT = {
  ...
  subtitle:
    "H\u1ecfi v\u1ec1 s\u00f3ng th\u1ecb tr\u01b0\u1eddng, s\u00f3ng ng\u00e0nh, chi\u1ebfn l\u01b0\u1ee3c",
  ready: "S\u1eb5n s\u00e0ng",
  expand: "M\u1edf r\u1ed9ng",
  placeholder:
    "H\u1ecfi v\u1ec1 s\u00f3ng th\u1ecb tr\u01b0\u1eddng, chi\u1ebfn l\u01b0\u1ee3c...",
  panelPlaceholder:
    "H\u1ecfi b\u1ea5t c\u1ee9 \u0111i\u1ec1u g\u00ec v\u1ec1 s\u00f3ng th\u1ecb tr\u01b0\u1eddng...",
...
function isPersistableMessage(message) {
  return (
    ["user", "ai"].includes(message?.role) && String(message?.text || "").trim()
  );
}
...
function loadChatHistory() {
  ...
  try {
    ...
    if (
      !saved?.savedAt ||
      Date.now() - Number(saved.savedAt) > CHAT_HISTORY_TTL_MS
    ) {
      window.localStorage.removeItem(CHAT_HISTORY_STORAGE_KEY);
      return [];
    }
    return normalizeStoredMessages(saved.messages);
  } catch {
    return [];
  }
}
...
function saveChatHistory(messages) {
  if (typeof window === "undefined") return;
  try {
    const cleanMessages = normalizeStoredMessages(messages);
    ...
    window.localStorage.setItem(
      ...
      JSON.stringify({ savedAt: Date.now(), messages: cleanMessages }),
    );
  } catch {
...
export default function TuVanAiCard() {
  ...
  const msgsDataRef = useRef(msgs);
  const activeRequestRef = useRef({ controller: null, id: 0 });
  const historyHydratedRef = useRef(false);

  useEffect(() => {
    msgsDataRef.current = msgs;
    if (historyHydratedRef.current) saveChatHistory(msgs);
    else historyHydratedRef.current = true;
    if (msgsRef.current)
  ...
  const setChatMessages = useCallback(
    (nextMsgs, { forcePanel = false, unsyncPanel = false } = {}) => {
      msgsDataRef.current = nextMsgs;
      setMsgs(nextMsgs);
      if (forcePanel || panelSyncedRef.current) setPanelMsgs(nextMsgs);
      if (forcePanel) {
  ...
  const sendMsg = useCallback(
    async (q, isPanel = false) => {
      const textToSend = q.trim();
      ...
      if (isPanel) setPanelVal("");
      else setChatVal("");

      const nextMsgs = [
        ...msgsDataRef.current.filter((m) => m.role !== "typing"),
        { role: "user", text: textToSend },
        { role: "typing", requestId },
      ];
      ...
      try {
        const result = await fetchReply(textToSend, controller.signal);
        if (
          controller.signal.aborted ||
          activeRequestRef.current.id !== requestId
        )
        ...
        if (
          ...
          return;
        finishWith(error.message || TEXT.apiError);
      } finally {
        if (activeRequestRef.current.id === requestId) {
          activeRequestRef.current = { controller: null, id: requestId };
          setLoading(false);
        }
      }
    },
      ...
      >
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <div>
              <div
                style={{ fontSize: 11, fontWeight: 600, color: "var(--t1)" }}
              >
                {TEXT.title}
              </div>
        ...
        <div
          style={{
            ...
            gap: 5,
            flexWrap: "wrap",
            borderTop: ".5px solid var(--bdr)",
            flexShrink: 0,
          }}
        >
          {SUGGEST_CHIPS.map((txt) => (
            <span
              key={txt}
              className="dm-chip"
              onClick={() => {
