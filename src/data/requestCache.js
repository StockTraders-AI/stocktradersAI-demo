...
const DATA_ROUTE_BY_PATH = new Map([
  ...
  [
    "/api/condition-signal-latest",
    { code: "r04", securePath: "/api/condition-signal-latest" },
  ],
  ["/api/do-song-advice", { code: "r05", securePath: "/api/do-song-advice" }],
  [
    "/api/do-song-recommendation",
    { code: "r06", securePath: "/api/do-song-recommendation" },
  ],
  [
    "/api/index-daily-changes",
    { code: "r07", securePath: "/api/index-daily-changes" },
  ],
  ...
  [
    "/api/smdt-branch-cross",
    { code: "r11", securePath: "/api/smdt-branch-cross" },
  ],
  ["/api/smdt-ticker", { code: "r12", securePath: "/api/smdt-ticker" }],
  [
    "/api/smdt-ticker-cross",
    { code: "r13", securePath: "/api/smdt-ticker-cross" },
  ],
  ...
  [
    "/api/stock-wave-current",
    { code: "r17", securePath: "/api/stock-wave-current" },
  ],
  [
    "/api/stock-wave-history",
    { code: "r18", securePath: "/api/stock-wave-history" },
  ],
  [
    "/api/stock-wave-tickers",
    { code: "r19", securePath: "/api/stock-wave-tickers" },
  ],
  ["/api/total-trade", { code: "r20", securePath: "/api/total-trade" }],
  [
    "/api/total-trade-real",
    { code: "r21", securePath: "/api/total-trade-real" },
  ],
  [
    "/api/wave-bottom-confirm-pairs",
    { code: "r22", securePath: "/api/wave-bottom-confirm-pairs" },
  ],
  [
    "/api/market/condition-signal-latest",
    { code: "r04", securePath: "/api/condition-signal-latest" },
  ],
  [
    "/api/market/do-song-advice",
    { code: "r05", securePath: "/api/do-song-advice" },
  ],
  [
  ...
  [
    ...
    { code: "r31", securePath: "/api/stock-wave-current" },
  ],
  [
    "/api/market/stock-wave-history",
    { code: "r32", securePath: "/api/stock-wave-history" },
  ],
  [
    "/api/market/stock-wave-tickers",
    { code: "r33", securePath: "/api/stock-wave-tickers" },
  ],
  [
    "/api/market/wave-bottom-confirm-pairs",
    { code: "r35", securePath: "/api/wave-bottom-confirm-pairs" },
  ],
]);
...
export async function fetchJsonWithClientCache(
  url,
  { force = false, ttlMs = DEFAULT_TTL_MS, options = {} } = {},
) {
...
export function fetchDataPostWithClientCache(url, params = {}, options = {}) {
  const { options: fetchOptions = {}, ...cacheOptions } = options || {};
  const postOptions = dataPostOptions(params);
  const mergedPostOptions = {
    ...postOptions,
    ...fetchOptions,
    headers: { ...postOptions.headers, ...(fetchOptions.headers || {}) },
  };
  const routeConfig = dataRouteForUrl(url);
  if (!routeConfig) {
    return fetchJsonWithClientCache(url, {
      ...cacheOptions,
      options: mergedPostOptions,
    });
  }

  const proxyPostOptions = dataPostOptions({ r: routeConfig.code, p: params });
  const mergedProxyPostOptions = {
    ...proxyPostOptions,
    ...fetchOptions,
    headers: { ...proxyPostOptions.headers, ...(fetchOptions.headers || {}) },
    secureUrl: routeConfig.securePath,
  };

  return fetchJsonWithClientCache(DATA_PROXY_URL, {
    ...cacheOptions,
    options: mergedProxyPostOptions,
  });
}
